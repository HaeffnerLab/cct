...
class Ey(QtGui.QWidget):
    ...
    def makeGUI(self):
        ...
        groupBox = QtGui.QGroupBox('Scan Ey and Tickle')
        ...
        self.label['minAmp'] = QtGui.QLabel('minimum Ey [V/m]:')
        self.control['maxAmp'].setRange(-10, 10)
        self.control['maxAmp'].setValue(DmaxAmp)	
        self.label['maxAmp'] = QtGui.QLabel('maximum Ey [V/m]:')
        self.control['numStepsAmp'].setValue(DnumStepsAmp)
        self.label['numStepsAmp'] = QtGui.QLabel('number of Ey steps:')
        self.control['sizeStepsAmp'].setValue(DsizeStepsAmp)
        self.label['sizeStepsAmp'] = QtGui.QLabel('Ey step size [V/m]:')
	...
	for Ey in amplitudes:
	    yield self.dv.cd(['', date, 'QuickMeasurements','MMComp', time + '-Ey and Tickle'],True)
	    name = yield self.dv.new('Ey: ' + str(Ey),[('Frequency', 'Hz')], [('PMT Counts', 'PMT counts', 'PMT counts')])
		...
		if self.running == False:
		    ...
		    yield self.ds.set_multipole_values([('Ex1', Ex), ('Ey1', Eyi), ('Ez1', Ez), ('U1', U1), ('U2', U2), ('U3', U3), ('U4', U4), ('U5', U5)])
...
class Scan_Control_Ey_and_Tickle(QtGui.QMainWindow):
    def __init__(self, reactor, parent=None):
        super(Scan_Control_Ey_and_Tickle, self).__init__(parent)
        self.reactor = reactor
        W = self.buildW(reactor)
        widget = QtGui.QWidget()
        gridLayout = QtGui.QGridLayout()    
        gridLayout.addWidget(W, 0, 0)
        self.setWindowTitle('Scan Ey and Tickle Control')
    ...
    def buildW(self, reactor):
        ...
        subLayout.addWidget(Ey(reactor), 0, 0)
...
if __name__=="__main__":
    ...
    Scan_Control_Ey_and_Tickle = Scan_Control_Ey_and_Tickle(reactor)
    Scan_Control_Ey_and_Tickle.show()
    reactor.run()   
