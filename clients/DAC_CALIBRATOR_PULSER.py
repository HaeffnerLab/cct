...
class DAC_CALIBRATOR(QDACCalibrator):
    def __init__(self,cxn, cam_cxn, parent=None):
        self.dacserver = cxn.cctdac_pulser
        self.dmmserver = cam_cxn.keithley_2100_dmm
...
if __name__=="__main__":
    import labrad
    cxn = labrad.connect()
    cam_cxn = labrad.connect('192.168.169.30')
    dacserver = cxn.cctdac_pulser
    dmmserver = cam_cxn.keithley_2100_dmm
    dmmserver.select_device('GPIB Bus - USB0::0x05E6::0x2100::1243106')
    app = QtGui.QApplication(sys.argv)
    icon = DAC_CALIBRATOR(cxn, cam_cxn)
    icon.show()
    app.exec_()
