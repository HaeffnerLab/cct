...
class CCT_GUI(QtGui.QMainWindow):
    ...
    def makeVoltageWidget(self, reactor):
        ...
        gridLayout.addLayout(subLayout,0,1, 1, 1)
        subLayout.addWidget(pmtWidget(reactor),0,0)
        subLayout.addWidget(Tickle_Control(reactor),1,0)      
        subLayout.addWidget(TD_CONTROL(reactor),2,0)
        subLayout.addWidget(SHUTTER(reactor), 3,0)        
        #subLayout.addWidget(Scan_Control_Ex_and_Tickle(reactor), 0, 1, 2, 1)
        #subLayout.addWidget(Scan_Control_Ey_and_Tickle(reactor), 2, 1, 2, 1)        
        widget.setLayout(gridLayout)
        return widget
