...
class CHANNEL_MONITOR(QtGui.QWidget):
    ...
    for j in range(16, 27):
        ...
        self.posDisplay.valueChanged.connect(self.dispChanged)
    ...
    def connect(self):
        ...
        yield self.setupListeners()
        yield self.followSignal(0, 0) 
        self.pos = yield self.dacserver.get_displacement_indices()       
    ...
    def slideChanged(self):
        if self.disp: return
        self.slide =  True
        self.newPos = self.slider.value() * 5
        self.disp.setValue()
        
    def dispChanged(self):     
