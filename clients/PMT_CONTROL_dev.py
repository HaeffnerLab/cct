...
class pmtWidget(QtGui.QWidget):
    ...
    def connect(self):
        ...
        self.server = cxn.normalpmtflow_dev
    ...
    def setupListeners(self):
        ...
        # yield self.server.signal__new_state(SIGNALID + 2)
        ...
        # yield self.server.addListener(listener = self.followState, source = None, ID = SIGNALID + 2)
    ...
    def on_toggled(self, state):
        if state:
            # yield self.server.enable_pmt( 0, True)
            ...
            # yield self.server.enable_pmt( 0, False)
            self.lcdNumber.display('OFF')
        self.setText(self.pushButton)
