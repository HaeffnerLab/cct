...
class excitation_729_with_multipole_ramp(base_excitation):
    from cct.scripts.PulseSequences.spectrum_rabi_with_multipole_ramp import spectrum_rabi_with_multipole_ramp
...
class excitation_blue_heat_rabi(base_excitation):
    from cct.scripts.PulseSequences.blue_heat_rabi import blue_heat_rabi
...
class excitation_ramsey_spectrum(base_excitation):
    from cct.scripts.PulseSequences.pulsed_scan_ramsey import pulsed_scan_ramsey
    name = 'ExcitationRamsey'
    pulse_sequence = pulsed_scan_ramsey

class excitation_dephase(base_excitation):
    from cct.scripts.PulseSequences.dephasing_chain import dephasing_chain
    name = 'DephasingChain'
    pulse_sequence = dephasing_chain
    
class excitation_ramsey_2ions(base_excitation):
    from cct.scripts.PulseSequences.ramsey_2ions import ramsey_2ions
    name = 'Ramsey2ions'
    pulse_sequence = ramsey_2ions

class excitation_rabi_2ions(base_excitation):
    from cct.scripts.PulseSequences.spectrum_rabi_2ions import spectrum_rabi_2ions
    name = 'Excitation2ions'
    pulse_sequence = spectrum_rabi_2ions

class excitation_ramsey_with_heating(base_excitation):
    #from cct.scripts.PulseSequences.dephasing_chain import dephasing_chain
    from cct.scripts.PulseSequences.ramsey_with_heating import ramsey_with_heating
    name = 'RamseyHeating'
    pulse_sequence = ramsey_with_heating
    
class ramsey_with_stark(base_excitation):
    from cct.scripts.PulseSequences.stark_shift_global import stark_shift_global
